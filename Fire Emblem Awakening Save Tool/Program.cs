...
namespace Fire_Emblem_Awakening_Save_Tool
{
    static class Program
    {
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Usage: FEAST <file1> [<file2> ...]");
            }
            for (int i = 0; i < args.Length; i++)
            {
                if (args[i].EndsWith("_dec")) // Decompressed file
                {
                    using (FileStream fs = new FileStream(args[i].Remove(args[i].Length - 4),
                        FileMode.Create, FileAccess.Write))
                    {
                        using (BinaryWriter bw = new BinaryWriter(fs))
                        {
                            byte[] raw = File.ReadAllBytes(args[i]);
                            bw.Write(raw, 0, 0xC0);
                ...
                {
                    int length = (int)(new FileInfo(args[i]).Length);
                    byte[] header = new byte[0xC0];
                    byte[] decompressed;
                    using (FileStream fs = new FileStream(args[i],
                    ...
                    using (FileStream fs = new FileStream(args[i] + "_dec",
                        FileMode.Create, FileAccess.Write))
                    {
                        using (BinaryWriter bw = new BinaryWriter(fs))
                        {
        ...
        static uint GetChecksum(byte[] data) // It's just CRC32.
        {
